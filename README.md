...
`G×EadjustedNMR` provides utilities for:

- Running `fastG×E` and `mmSuSiE` from R by the `run_fastgxe()` function.

> **Note:** When the genotype data are too large, invoking `fastG×E` and `mmSuSiE` from R may fail due to excessive memory demands. In such cases, we recommend using the original standalone implementations of `fastG×E` and `mmSuSiE` directly. See <https://github.com/chaoning/mmsusie> for more details.

- Selecting significant main-effect SNPs by \(select_main_snps\) function.
- Selecting and LD-clumping G×E SNPs by \(clump_gxe_snps\) function.
- Extracting genotypes using PLINK by \(extract_genotypes_plink\) function.
- Constructing G×E adjustment scores by \(compute_main_prs\) function.
- Constructing G×E-adjusted exposures by \(compute_gxe_adjustment\) function.
- Running doubly-ranked stratification by \(create_nlmr_summary\) function.
- Plotting by \(plot_lace\) and \(plot_heter\) function.
